...
def eval_expro(expr, env, value, depth=0, maxdepth=3):
    ...
    return (conde,
        ...
        ((eq, expr, ast.BinOp(left=var("mod_e1_" + uuid), op=ast.Mod(), right=var("mod_e2_" + uuid))),
         (typeo, var("mod_v1_" + uuid), int),
         (typeo, var("mod_v2_" + uuid), int),
        ...
        ((typeo, exprs, list),
         ...
         (conde,
          ...
          ((conso, head_expr, tail_expr, exprs),
           ...
           (conso, head_value, tail_value, tuple_value),
           # LCons is reified to a tuple, hence the extra step
           # TODO: Maybe make LCons nicer or get rid of LCons?
